...
export const atomWithInfiniteQuery = <Data>(
  ...
  const baseAtom = atom((get) => {
    const options =
      typeof queryOptions === 'function' ? queryOptions(get) : queryOptions;
    ...
    const initialData = asInfiniteData(
      get(theInitialDataAtom) as unknown as Data,
    );
  ...
  const anAtom = atom<
    InfiniteData<Data> | undefined,
    AtomWithInfiniteQueryAction<Data>
  >(
    (get) => {
