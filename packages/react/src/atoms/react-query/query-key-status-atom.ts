...
export const queryStatusAtomFamily = atomFamily<
  QueryKey,
  QueryStatus,
  QueryStatus
>(
  (queryKey) =>
    atomWithDefault<QueryStatus>((get) => {
      ...
      const { isFetching, isLoading, isIdle, isSuccess, isStale } =
...
export const queryKeyStatusAtom = atomFamily<QueryKey, QueryStatus>(
  (queryKey) => {
